...
def check_mcp_server(silent=False):
    """1. Verificar si el servidor MCP está levantado"""
    url = "http://localhost:8003/health"
    try:
        with urllib.request.urlopen(url, timeout=2) as response:
            if response.status == 200:
                if not silent: print("Servidor MCP activo")
                return True
    except:
        pass
...
def verify_mcp_mode(silent=False):
...
def validate_llm_configuration(silent=False):
    ...
    if not silent: print(f"Validando Proveedor: {provider}")
    ...
    if provider == "google":
        ...
        if result.returncode == 0:
            if not silent: print(f"Proveedor Gemini, Modelo: {model}")
        else:
        ...
        result = subprocess.run([str(VENV_PYTHON), str(script_path)], capture_output=True, text=True)
        if result.returncode == 0:
            if not silent: print(f"Proveedor openai Modelo: {model}")
        else:
...
def run_main_test(limit=None, resume_mode=False):
    ...
    if resume_mode:
        cmd.append("--resume")
    if limit:
        if not resume_mode: print(f"Limitando a los primeros {limit} casos de prueba.")
        cmd.append(str(limit))
    
    subprocess.run(cmd, env=env)

def check_erp_mock(silent=False):
    """1.5 Verificar si el ERP Mock está levantado (Puerto 8001)"""
    url = "http://localhost:8001/health"
    if not silent: print(f"Verificando ERP Mock en {url} (PUERTO HARDCODEADO 8001)...")
    try:
        with urllib.request.urlopen(url, timeout=2) as response:
            if response.status == 200:
                if not silent: print("✅ Servidor ERP Mock activo")
                return True
    except:
        pass
...
def main():
    ...
    # 1. Check MCP
    if not check_mcp_server(silent=resume_mode):
        sys.exit(1)

    # 1.5 Check ERP Mock
    if not check_erp_mock(silent=resume_mode):
        sys.exit(1)
        
    # 2. Check MCP Mode
    verify_mcp_mode(silent=resume_mode)
    
    # 3. Validate LLM
    if not validate_llm_configuration(silent=resume_mode):
